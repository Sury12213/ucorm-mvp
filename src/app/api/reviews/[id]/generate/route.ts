...
function normalizeSuggestions(value: unknown): GeneratedSuggestion[] | null {
  ...
  const normalized = suggestions.map((suggestion) => {
    ...
    if (
      !expectedTones.includes(tone as SuggestionTone) ||
...
export async function POST(
  ...
  if (!review.text) {
    return NextResponse.json(
      { message: "Đánh giá không có nội dung" },
      { status: 400 },
    );
  }
    ...
    messages: [
      {
        role: "system",
        content:
          "Return valid JSON only. Reply in review language. Each content max 2 sentences.",
      },
      {
        role: "user",
        content: `Rating: ${review.rating ?? "N/A"}/5\nReview: ${review.text}\n\nReturn {"suggestions":[{"tone":"Standard","content":""},{"tone":"Friendly","content":""},{"tone":"Recovery","content":""}]}`,
      },
    ],
    max_tokens: 300,
  });
  ...
  if (!parsed) {
    return NextResponse.json(
      { message: "Định dạng phản hồi AI không hợp lệ" },
      { status: 502 },
    );
  }
