  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          <h4 className="font-headline text-base font-bold text-on-surface">{toneLabels[suggestion.tone]}</h4>
          <p className="mt-1 text-xs leading-5 text-on-surface-variant">{toneDescriptions[suggestion.tone]}</p>
        </div>
        {approved && (
      ...
      {editing ? (
        ...
        <p className="rounded-xl bg-surface-container-low p-4 text-sm leading-6 text-on-surface-variant">{content}</p>
      )}
      ...
      {onApprove && !approved && (
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          >
            ...
            {editing ? 'Xong chỉnh sửa' : 'Chỉnh sửa'}</button>
          <button
            type="button"
